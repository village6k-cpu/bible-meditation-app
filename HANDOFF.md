...
### 2026-09-22 최신 작업 — 다른 계정에 묶인 빈 기록함 복구

- Studio Mac의 새 기록함에서 먼저 다른 Google 계정으로 로그인한 뒤 `village.6k@gmail.com`으로
  다시 로그인해도, 로컬 `sync_meta.account_id`가 첫 계정에 고정되어 「이 기록함은 다른 계정에
  연결되어 있습니다」로 수신이 막히는 실제 화면을 확인했다. 올바른 계정의 서버 기록 10건은
  전용 Ledger 프로젝트에 남아 있고 RLS·프로젝트 상태도 정상이다.
- 기존 보호 장치는 계정 혼합을 막기만 하고 새 기록함으로 바꾸는 실행 경로를 제공하지 않았다.
  계정 불일치를 전용 오류로 구분하고 설정에 **현재 계정 기록 불러오기**를 추가했다.
  사용자가 확인하면 먼저 현재 SQLite 파일을 전달한 뒤, 로그인 계정이 그대로인지 다시 확인한다.
  그 후 옛 계정에서 받은 로컬 동기화 캐시와 큐만 한 트랜잭션으로 비우고 계정·수신 커서를 바꿔
  revision 0부터 현재 계정 기록을 받는다. 로컬 사진 파일 자체는 지우지 않는다.
- 일반 자동 동기화에서는 기존 계정 혼합 차단을 그대로 유지한다. 백업 취소·실패, 로그인 계정 변경,
  로컬 연결 상태 변경 시에는 교체를 실행하지 않는다. HeyBilly·Hermes·Supabase 설정·DB 권한·
  Google 설정은 변경하지 않았다.
- 실패 재현 후 회귀 테스트를 추가했다. 옛 계정의 삭제 기록·출처·갈피가 새 계정으로 전송되지 않고,
  수신 커서 0에서 올바른 계정 기록을 받는 것을 실제 SQLite로 확인했다. 웹 56개, 코어 113개,
  양쪽 타입 검사와 웹 프로덕션 빌드가 통과했다. 로컬 화면 자동 확인 도구는 `runtime_unavailable`로
  중단되어 390px/1280px 시각 확인과 실제 Studio Mac의 10건 수신은 배포 후 남아 있다.

### 2026-09-13 최신 작업 — 유형 바로잡기·사진 편집

- 배포 완료: PR #15 구현 `0ff83e2`, main 머지 `6baa4eb`. PR CI `34724331934` 성공,
  Pages 배포 `34724399851` 성공. 빌드 `b74893a1825f`, 운영 자산 `main-DPAYBFPc.js` 확인.
  운영 Chrome 재실행 후 기존 H 본문·갈피·180px 사진과 `동기화 완료`, 편집의 유형 선택/사진 버튼을
  실제 확인하고 스크린샷을 검토했다. 물리적 iPhone의 새 편집 조작은 직접 확인한 것이 아니다.
