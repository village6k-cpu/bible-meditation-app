...
찍힌다(정리할 것이 없다). 화면은 `web/src/ui/parts/practice.tsx`의 `layoutDay`/`Cell`이 그린다 —
수집함 맨 위 한 줄, 지표의 28일 격자. 지난 날의 빈 칸은 채우지 못하게 둔다(꾸밈이 된다).
캡처는 실천이면 저장 뒤 시트를 닫는다(이어서 적을 것이 없다).
