...
export async function getDailyReadings(date: string): Promise<DailyReading[]> {
  const readings = await getUserDb().getAllAsync<DailyReading>(
    'SELECT * FROM daily_readings WHERE date = ? ORDER BY id',
    [date]
  );
  // Enrich with book names from bible.db
...
export async function getAllNotes(): Promise<Note[]> {
  const notes = await getUserDb().getAllAsync<Note>(
...
export async function getNote(id: number): Promise<Note | null> {
  const note = await getUserDb().getFirstAsync<Note>('SELECT * FROM notes WHERE id = ?', [id]);
  if (note?.book_id) {
