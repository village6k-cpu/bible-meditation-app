...
export default function BoardScreen() {
  ...
  useFocusEffect(useCallback(() => {
    if (gid > 0) {
      getAllPosts(gid, category).then(setPosts);
    }
  }, [gid, category]));
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{decodeURIComponent(groupName ?? '게시판')}</Text>
        <View style={{ width: 22 }} />
      </View>
      ...
      <View style={styles.tableHeader}>
        <Text style={[styles.th, styles.thNum]}>번호</Text>
        <Text style={[styles.th, styles.thTitle]}>제목</Text>
        <Text style={[styles.th, styles.thAuthor]}>글쓴이</Text>
            ...
            return (
              ...
              >
                ...
                <View style={styles.cellTitleWrap}>
                  ...
                  <Text style={styles.cellTitle} numberOfLines={1}>{post.title}</Text>
      ...
      <TouchableOpacity
        style={styles.fab}
        onPress={() => router.push(`/post/write?groupId=${gid}`)}
        activeOpacity={0.8}
      >
...
const styles = StyleSheet.create({
  ...
  catTab: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 16, backgroundColor: colors.surface },
  ...
  th: { fontFamily: fonts.sansSemiBold, fontSize: 11, color: colors.textSecondary },
